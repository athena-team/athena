# coding=utf-8
# Copyright (C) 2019 ATHENA AUTHORS; Xiangang Li; Shuaijiang Zhao; Yanguang Xu
#
...
class SpeechRecognitionDatasetBuilder(SpeechBaseDatasetBuilder):
    ...
    def preprocess_data(self, file_path):
        ...
        if self.hparams.remove_unk and unk != -1 and self.hparams.ignore_unk is True:
    ...
    def storage_features_offline(self):
        feat_dict = {}
        label_dict = {}
        os.makedirs(self.hparams.data_scps_dir, exist_ok=True)
        with open(self.hparams.data_scps_dir + "/" + "feat_shape", 'w') as feat_shape_file, \
             open(self.hparams.data_scps_dir + "/" + "label_shape", 'w') as label_shape_file:
            for key, _, transcripts, speed, speaker in self.entries:
                ...
                # do spectral_augmentation in SpeechRecognitionDatasetKaldiIOBuilder.__getitem__()
                # if self.hparams.spectral_augmentation is not None:
                ...
                if str(speed) == "1.0":
                    ...
                    key = os.path.splitext(os.path.basename(key))[0] + "_" + str(speed)
                feat_dict[key] = feat
                label_dict[key] = np.array(label, dtype='int32')
                feat_shape_file.write(key + '\t' + speaker + '\t' + str(feat.shape[0] * feat.shape[1] * 2) + '\n')
                label_shape_file.write(key + '\t' + speaker + '\t' + str(len(label) * 2) + '\n')
    ...
    def __getitem__(self, index):
        ...
        utt_id = os.path.basename(audio_data)
        return {
            "input": feat,
            "input_length": feat_length,
            "output_length": label_length,
