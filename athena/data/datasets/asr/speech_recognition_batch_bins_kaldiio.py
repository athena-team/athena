# coding=utf-8
# Copyright (C) 2019 ATHENA AUTHORS; Xiangang Li; Shuaijiang Zhao; Ne Luo; Yanguang Xu; Xuezheng Deng
#
...
class SpeechRecognitionDatasetBatchBinsKaldiIOBuilder(SpeechRecognitionDatasetKaldiIOBuilder):
    ...
    def preprocess_kaldi_data(self, file_dir, apply_sort_filter=True):
        super().preprocess_kaldi_data(file_dir, apply_sort_filter=apply_sort_filter)
        self.speech_shape_dict = self.read_shape_file(
            self.hparams.data_scps_dir+'/'+'feat_shape')
        self.text_shape_dict = self.read_shape_file(
            self.hparams.data_scps_dir+'/'+'label_shape')

        speech_shape = dict(
            filter(
                lambda e: e[1][0] in range(
                    self.hparams.input_length_range[0] // 10 *
                    self.hparams.audio_config["filterbank_channel_count"] * 2,
                    self.hparams.input_length_range[1] // 10 *
                    self.hparams.audio_config["filterbank_channel_count"] * 2
                ),
                self.speech_shape_dict.items()
            ))
        text_shape = dict(
            filter(
                lambda e: e[1][0] in range(
                    self.hparams.output_length_range[0] * 2,
                    self.hparams.output_length_range[1] * 2
                ),
                self.text_shape_dict.items()
            ))
        filter_keys = set(self.speech_shape_dict.keys())-set(speech_shape.keys()
                                                             ) | set(self.text_shape_dict.keys())-set(text_shape.keys())
        for filter_key in filter_keys:
            del self.speech_shape_dict[filter_key]
            del self.text_shape_dict[filter_key]

        shape_files = ["feat_shape", "label_shape"]
    ...
    def read_shape_file(self, file_dir=None):
        file = open(file_dir, 'r')
        dec = {}
        for line in file:
            key, speaker, value = line.strip('\n').split('\t')
            dec[tuple([key, speaker])] = [int(value)]
        return dec
    ...
    def __getitem__(self, index):
        ...
        label_length = []
        utts = []
        for key, speaker in self.batchs_list[index]:
            try:
