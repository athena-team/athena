...
from athena.loss import FastSpeechLoss
    ...
    def __init__(self, data_descriptions, config=None):
        ...
        encoder_layers = [
            TransformerEncoderLayer(self.hparams.d_model, self.hparams.num_heads,
                                    self.hparams.dff, self.hparams.rate, ffn=ffn_list[0], conv_module_kernel_size=self.hparams.conv_module_kernel_size)
        ...
        if self.n_speakers > 1:
                ...
                self.speaker_fc = tf.keras.layers.Dense(units=self.hparams.d_model, name="speaker_fc")                                          
        
        # define decoder
        decoder_layers = [
            TransformerEncoderLayer(self.hparams.d_model, self.hparams.num_heads,
                                    self.hparams.dff, self.hparams.rate, ffn=ffn_list[1], conv_module_kernel_size=self.hparams.conv_module_kernel_size)
