...
from athena.loss import Tacotron2Loss, GuidedAttentionLoss
from athena.layers.commons import ZoneOutCell
from athena.layers.attention import LocationAttention, StepwiseMonotonicAttention, MultiHeadAttention
from athena.layers.functional import collapse4d
...
class Tacotron2(BaseModel):
    ...
    default_config = {
        ...
        "speaker_embedding_integration_type": "add", # add || concat
    ...
    def __init__(self, data_descriptions, config=None):
        ...
        self.n_speakers = len(data_descriptions.speakers)
        if self.n_speakers > 1:
                self.speaker_embedding = layers.Embedding(self.n_speakers, self.hparams.speaker_embedding_dim)
                self.speaker_fc = tf.keras.layers.Dense(units=self.hparams.eunits, name="speaker_fc")         
    ...
    def call(self, samples, training: bool = None):
        ...
        if self.n_speakers > 1:
            ...
            speaker_embedding_init = self.speaker_embedding(speaker)
            speaker_embedding = tf.nn.l2_normalize(speaker_embedding_init, axis=-1)

            if self.hparams.speaker_embedding_integration_type == "add":
                speaker_embedding_reshape = tf.reshape(speaker_embedding,[batch, 1, self.hparams.speaker_embedding_dim])
                speaker_embedding_final = self.speaker_fc(speaker_embedding_reshape)
                encoder_output += speaker_embedding_final
            else:
