# coding=utf-8
# Copyright (C) 2022 ATHENA AUTHORS; Jianwei Sun
# All rights reserved.
#
...
class KWSAVTransformer(BaseModel):
    ...
    def __init__(self, data_descriptions, config=None):
        ...
        v_net
        """
        input_vfeature = layers.Input(
            shape=data_descriptions.sample_shape["video"],
            dtype=tf.float32)

        # tf.ensure_shape(input_features, [None, None, 63, 1])

        vinner = layers.Conv2D(
            filters=self.hparams.num_filters,
            kernel_size=(3, 3),
            strides=(1, 4),
            padding="same",
        ...
        vinner = layers.BatchNormalization()(vinner)
        vinner = tf.nn.relu6(vinner)
        vinner = layers.Conv2D(
            filters=self.hparams.num_filters,
            kernel_size=(3, 3),
            strides=(1, 2),
        ...
        vinner = tf.nn.relu6(vinner)
        _, _, dim, channels = vinner.get_shape().as_list()
        output_dim = dim * channels
        vinner = layers.Reshape((-1, output_dim))(vinner)
        vinner = layers.Dense(self.hparams.d_model, activation=tf.nn.relu6)(vinner)
        self.x_net = tf.keras.Model(inputs=input_features, outputs=inner, name="x_net")
        self.v_net = tf.keras.Model(inputs=input_vfeature, outputs=vinner, name="v_net")
        logging.info(self.v_net.summary())
    ...
    def call(self, samples, training=None):
        x0 = samples["input"]
        v0 = samples["video"]
        a = self.x_net(x0, training=training)
