...
class KWSTransformerRESNET(BaseModel):
    ...
    def __init__(self, data_descriptions, config=None):
        ...
        resnetmodel = tf.keras.applications.resnet.ResNet152(include_top=False, weights='imagenet', input_tensor=None,
                                                             input_shape=[None, 63, 3], pooling=None)
        self.resnet_conv = tf.keras.Model(inputs=resnetmodel.input, outputs=resnetmodel.layers[100].output,name="resnet")
        logging.info(self.resnet_conv.summary())
        ...
        input_features = layers.Input(
                shape=[None, 8, 512],
                dtype=tf.float32)
        inner = layers.BatchNormalization()(input_features)
    ...
    def call(self, samples, training=None):
        x0 = samples["input"]
        x1 = tf.concat([x0,x0,x0],axis = 3)
        x = self.resnet_conv(x1,training=False)
