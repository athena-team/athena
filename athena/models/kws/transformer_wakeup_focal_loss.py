...
class KWSTransformer_FocalLoss(BaseModel):
    ...
    def get_loss(self, outputs, samples, training=None):
        """ get loss """
        y_pred = tf.math.sigmoid(outputs, name="sigmoid")
        y_pred = tf.cast(y_pred, tf.float32)
        y_true = samples["output"]
        y_true = tf.cast(y_true, tf.float32)

        p_t = (1 - y_pred) * y_true + y_pred * (1 - y_true)
        focal_weight = (self.alpha * y_true + (1 - self.alpha) *
                        (1 - y_true)) * tf.pow(p_t, self.gamma)
        loss = self.loss_function(samples["output"], outputs, sample_weight=focal_weight)
        self.metric(samples["output"], outputs)
