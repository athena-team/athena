...
class KWSTransformer_AveragePooling(BaseModel):
    ...
    def __init__(self, data_descriptions, config=None):
        ...
        self.dense_layer = layers.Dense(self.hparams.d_model, input_shape=(self.hparams.d_model,))
    ...
    def call(self, samples, training=None):
        ...
        x_ = tf.keras.layers.AveragePooling1D(
            pool_size=None,
            strides=1,
            padding='valid',
            data_format='channels_last',
        )(x) 
        y_ = self.dense_layer(x_)
        y = self.final_layer(y_)
        return y
