...
class WarmUpLearningSchedule1(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __init__(self, model_dim=512, warmup_steps=4000, k=1.0,
                 decay_steps=99999999, decay_rate=1.0, lr=None):
        """parameters for warmup learning rate schedule
        ...
        Args:
            ...
            warmup_steps (int, optional): learning rate increases slowly till warmup_steps. Defaults to 4000.
            decay_steps (int, optional): learning rate decay starts after decay_steps. Defaults to 99999999.
            lr (int, optional): the initial learning rate value. Defaults to None.
        """
        super().__init__()
    ...
    def __call__(self, step):
        ...
        if self.lr == None:
            ...
            return tf.minimum(k * tf.math.rsqrt(self.model_dim) * tf.math.minimum(arg1, arg2), self.lr)


class WarmUpAdam1(tf.keras.optimizers.Adam):
    """WarmUpAdam Implementation"""
    default_config = {
        "d_model": 512,
        "warmup_steps": 8000,
    ...
    def __init__(self, config=None, beta_1=0.9, beta_2=0.999, epsilon=1e-7,
        ...
        self.hparams = register_and_parse_hparams(self.default_config, config, cls=self.__class__)
        super().__init__(
            learning_rate=WarmUpLearningSchedule1(
                self.hparams.d_model,
                self.hparams.warmup_steps,
                self.hparams.k,
                self.hparams.decay_steps,
